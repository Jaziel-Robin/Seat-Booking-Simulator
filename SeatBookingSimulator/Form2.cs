...
namespace SeatBookingSimulator
{
    public partial class Form2 : Form
    {
        ...
        private void ButtonSafeDistance_Click(object sender, EventArgs e)
        {
            TextBox_MaxSeats.Enabled = false;
            TextBox_MaxSeats.BackColor = Color.DarkGray;
            string columns = TextBox_Col.Text;
            int col;
            try
            {
                col = int.Parse(columns);
            }
            catch (FormatException ex)
            {
                Console.WriteLine(ex.Message);
                MessageBox.Show("Ensure that seats have been generated before clicking this button.");
                return;
            }
            int colDisabled;
            int colEnabled;
            ...
            foreach (Control c in this.panelSeats.Controls)
            {
                ...
                if (seat.Row % 2 == 1)
                {
                    ...
                    for (int i = 0; i < col; i++)
                    {
                        ...
                        if (seat.Column == colDisabled)
                        {
                            if (seat != null && seat.BookStatus == false)
                            {
                                seat.CanBook = false;
                                c.Enabled = true;
                                c.BackColor = Color.LightPink;
        ...
        }//End of ButtonSafeDistance_Click
        ...
        private void LabelSeat_Click(object sender, EventArgs e)
        {
            ...
            if (seat.BookStatus == false)
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("You cannot choose a seat which is next to another seat booked by another person!");
                        return;
                    }
